...
public class OpenUDID_manager implements ServiceConnection{
		...
		try {
			...
			if (data.readInt() == reply.readInt()) //Check if the service returns us this number
			{
				...
				if (_openUDID != null) { //if valid OpenUDID, save it
					if (LOG) SSLog.d(TAG, "Received " + _openUDID);
	...
	private void generateOpenUDID() {
		if (LOG) SSLog.d(TAG, "Generating openUDID");
	...
	private void startService() {
		if (mMatchingIntents.size() > 0) { //There are some Intents untested
			if (LOG) SSLog.d(TAG, "Trying service " + mMatchingIntents.get(0).loadLabel(mContext.getPackageManager()));
			...
			if (OpenUDID == null) //No OpenUDID was chosen, generate one			
				generateOpenUDID();
			if (LOG) SSLog.d(TAG, "OpenUDID: " + OpenUDID);

			storeOpenUDID();//Store it locally
			mInitialized = true;
			
			if(_initCompletedHandler != null) {
				_initCompletedHandler.openUDIDInitCompleted();
			}
		}
	}
	...
	private IOpenUDIDInitCompleted _initCompletedHandler = null;
	
	public IOpenUDIDInitCompleted getInitCompletedHandler() {
		return _initCompletedHandler;
	}

	public void setInitCompletedHandler(IOpenUDIDInitCompleted initCompletedHandler) {
		_initCompletedHandler = initCompletedHandler;
	}
	 ...
	 */
	public static void sync(Context context, IOpenUDIDInitCompleted initCompleted) {
		//Initialize the Manager
		OpenUDID_manager manager = new OpenUDID_manager(context);
		manager.setInitCompletedHandler(initCompleted);
		...
		if (OpenUDID == null) //Not found
		{
			...
			if (LOG) SSLog.d(TAG, manager.mMatchingIntents.size() + " services matches OpenUDID");
			...
			if(manager.getInitCompletedHandler() != null) {
				manager.getInitCompletedHandler().openUDIDInitCompleted();
			}
